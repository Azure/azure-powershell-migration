function Invoke-AzUpgradeModulePlan
{
    ...
    .PARAMETER FileEditMode
        Specifies the file edit mode to determine if the upgrade plan should be executed in place (modifies existing files) or if changes should be saved to new files. Specify 'ModifyExistingFiles' to modify your script files in place,
        or 'SaveChangesToNewFiles' to save new PowerShell files (leaving your existing files unmodified).

    .EXAMPLE
        The following example invokes the upgrade plan for a PowerShell module named "myModule" and saves the updated scripts to new files (leaving original files unmodified).
