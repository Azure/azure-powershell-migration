...
export async function updateDiagnostics(
    ...
    if (documentUri) {
        ...
        try {
            log.write(
                `Start analyzing ${documentUri.fsPath} by Az.Tools.Migration.`
            );
            ...
            const settingPSA = path.resolve(
                __dirname,
                '../PSA_custom_Rules/CustomRules.psm1'
            );
            log.write(
                `Start analyzing ${documentUri.fsPath} by PowershellScriptAnalyzer custom rules.`
            );
            PSAResult = await powershell.getCustomSuggestions(
                documentUri.fsPath,
                settingPSA
            );
...
function formatPlanstToDiag(
    ...
    plans.forEach((plan) => {
        const range = new vscode.Range(
            new vscode.Position(
                plan.SourceCommand.StartLine - 1,
                plan.SourceCommand.StartColumn - 1
            ),
            new vscode.Position(
                plan.SourceCommand.EndLine - 1,
                plan.SourceCommand.EndPosition - 1
            )
        );
 ...
 * @returns : diagnostics
 */
function formatPsaSuggestsToDiag(
    plansStr: string,
    log: Logger,
    diagnostics: vscode.Diagnostic[]
): vscode.Diagnostic[] {
    ...
    plans.forEach((plan) => {
        const range = new vscode.Range(
            new vscode.Position(
                plan.StartLineNumber - 1,
                plan.StartColumnNumber - 1
            ),
            new vscode.Position(
                plan.EndLineNumber - 1,
                plan.EndColumnNumber - 1
            )
        );
